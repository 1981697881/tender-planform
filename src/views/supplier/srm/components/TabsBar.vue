  ...
  methods: {
    ...
    Delivery() {
      if (this.clickData.id) {
        this.$confirm('是否删除（' + this.clickData.supplierName + '），删除后将无法恢复?', '提示', {
          ...
          this.$emit('delList', {
            ...
            supplierAccountNumber: this.clickData.supplierAccountNumber
          })
        }).catch(() => {
          this.$message({
            type: 'info',
    ...
    qFilter() {
      let obj = {}
      this.search.name != null && this.search.name != '' ? obj.supplierName = this.search.name : null
      return obj
    },
    handleAdd(){
