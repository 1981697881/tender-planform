    ...
    <el-form :model="form" :rules="rules" ref="form" label-width="130px" :size="'mini'">
      <el-row :gutter="20">
        <div style="margin-top: 20px;margin-bottom: 10px">
          <el-button @click="setRow" type="success" size="small">选择</el-button>
        </div>
...
import {choiceRegistrations,registrationsList} from '@/api/tender/index'
...
export default {
  ...
  data() {
    return {
      ...
      columns2: [{text: '供应商名称', name: 'supplierName'}, {text: '是否中标', name: 'choiceRegistrations',formatt: 'checkStatus'}],
      rules: {},
      rules2: {},
    };
  },
  ...
  methods: {
    checkStatus(row, column) {
      let stau = ''
      if(row.choiceRegistrations == 0) {
        stau = '否'
      }else if(row.choiceRegistrations ==1) {
        stau = '是'
      }
      return  stau
    },
    handleClick(val) {
      choiceRegistrations({id: val.rid, pojectNo: this.form.pojectNo, choiceRegistrations: '1'}).then(res => {
        if (res.flag) {
          this.visible = false
          this.visible2 = false
